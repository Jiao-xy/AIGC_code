...
def shuffle_text(text, shuffle_sentences=True, shuffle_phrases=True, shuffle_words=True):
    # **1. 句子级别打乱**
    sentences = re.split(r'(?<=[。！？\n])', text)  # 以标点拆分句子
    if shuffle_sentences:
        random.shuffle(sentences)

    # **2. 短语级别打乱**（以逗号、分号等为界拆分）
    if shuffle_phrases:
        sentences = ["，".join(random.sample(s.split("，"), len(s.split("，")))) for s in sentences if s]

    # **3. 单词级别打乱**（以空格拆分）
    if shuffle_words:
        sentences = [" ".join(random.sample(s.split(), len(s.split()))) for s in sentences if s]

    return "".join(sentences)  # 重新组合成文本
...
"""
# 生成打乱的文本
shuffled_text = shuffle_text(text1)
print(f"原始文本: {text1}")
print(f"打乱文本: {shuffled_text}")
# 计算相似度
embedding_original = model.encode(text1, convert_to_tensor=True)
embedding_shuffled = model.encode(shuffled_text, convert_to_tensor=True)
similarity = util.pytorch_cos_sim(embedding_original, embedding_shuffled).item()

print(f"打乱后相似度: {similarity}")  # 预期 < 0.5
